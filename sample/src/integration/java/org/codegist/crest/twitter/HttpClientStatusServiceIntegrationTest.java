 ...
 */
public class HttpClientStatusServiceIntegrationTest extends AbstractStatusServiceIntegrationTest {
    public HttpClientStatusServiceIntegrationTest() {
        super(getAccountService(ACC_1), getAccountService(ACC_2));
    }

    private static AccountService<StatusService> getAccountService(String account){
        return new AccountService<StatusService>(
                CRestBuilderFactory.getBaseCRestBuilder(account).useHttpClientRestService().build().build(StatusService.class),
